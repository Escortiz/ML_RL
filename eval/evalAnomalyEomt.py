...
def main():
    ...
    parser.add_argument('--config', default="/content/ML_RL/eomt/configs/dinov2/cityscapes/semantic/eomt_base_640.yaml")
    parser.add_argument('--model_name', type=str, default='eomt')
    ...
    if 'encoder.backbone.pos_embed' in clean_state_dict:
        ...
        if ckpt_pos.shape != model_pos.shape:
            ...
            ckpt_pos_resized_2d = F.interpolate(ckpt_pos_2d, size=(H_grid, W_grid), mode='bicubic', align_corners=False)
    ...
    for path in glob.glob(os.path.expanduser(str(args.input[0]))):
        ...
        with torch.no_grad():
            ...
            prob_queries = torch.nn.functional.softmax(pred_logits / args.temperature, dim=-1)
            ...
            mask_weights = torch.nn.functional.softmax(pred_masks, dim=0) 
            
            logit_map = torch.einsum("qc,qhw->chw", known_logits, mask_weights)
